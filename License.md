```
Terms of Service

About ABCD (AIST Building Change Detection) Dataset

June 30, 2017

ABCD dataset is based on results obtained from a project commissioned by the New Energy and Industrial Technology Development Organization (NEDO). ABCD dataset is permitted of use only users who comply with the following terms of service.

1. Users are permitted to use ABCD dataset only for noncommercial use.

2. In the case of publication as a paper, magazine, or other external material, the user shall specify the following matters in the corresponding document.

 Specify the URL of ABCD dataset.

 Specify "Copyright of aerial images are PASCO corporation" as copyright of aircraft photograph data provider.

3. At the time of downloading, we treat the user as agreeing to these terms of service.

National Institute of Advanced Industrial Science and Technology

Artificial Intelligence Research Center, Japan
```
