...
We prepared the patch pairs for two types of size: **fixed-scale** and **resized**. Fixed-scale patches have the same resolution of the original images (40 cm) and the patch size is 160 x 160 pixels. In contrast, resized patches do not keep the original resolution, but were cropped depending on the size of each target building (three times larger than the target building); so the patch size varies from building to building. Lastly, resized patches are the resized version of size-adaptive patches, specifically resized to 120 x 120 pixels.  
